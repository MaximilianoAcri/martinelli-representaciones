...
export default function ProspectosPage() {
  ...
  const getMensajeWA = (nombre: string) =>
    `Hola ${nombre}! 👋 Te contacto de *Martinelli Representaciones*. Somos representantes de productos para construcción (chantas, mallas, etc.) y me gustaría acercarte nuestro catálogo y precios. ¿Tenés un momento?`;

  // Abrir WhatsApp con mensaje pre-llenado + marcar como contactado
  const contactarProspecto = async (prospecto: Prospecto) => {
    const mensaje = getMensajeWA(prospecto.nombre);
    window.open(`https://wa.me/${prospecto.telefonoLimpio}?text=${encodeURIComponent(mensaje)}`, "_blank");
    // Auto-marcar como contactado
    if (prospecto.estado === 'nuevo') {
      await actualizarEstado(prospecto.id, 'contactado');
    }
  };

  // Modo Campaña: ir prospecto por prospecto
  ...
  const iniciarCampana = () => {
    ...
    if (nuevos.length === 0) {
      alert('No hay prospectos nuevos con teléfono para contactar');
      return;
    }
    // Filtrar solo nuevos para la campaña
  ...
  const contactarSeleccionados = () => {
    const prosp = prospectosFiltrados.filter(p => seleccionados.has(p.id) && p.telefonoLimpio);
    if (prosp.length === 0) {
      alert('No hay teléfonos válidos en la selección');
      return;
    }
    contactarProspecto(prosp[0]);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      <main className="max-w-7xl mx-auto px-2 sm:px-4 py-4">
        ...
        <div className="flex flex-wrap gap-1 justify-between mb-2">
          <div className="flex flex-wrap gap-1">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded text-sm font-semibold"
              title="Contactar prospectos nuevos uno por uno"
            >
              🚀 Modo Campaña
            </button>
          </div>
            ...
            {prospectosPaginados.map((prospecto) => (
              ...
              >
                ...
                <div className="col-span-2">
                  <button
                    onClick={() => contactarProspecto(prospecto)}
                    className="text-green-400 hover:text-green-300 font-semibold text-left hover:underline"
        ...
        {/* MODO CAMPAÑA */}
        {modoCampana && prospectoActualCampana && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
            <div className="w-full max-w-md bg-slate-800 rounded-2xl border border-slate-600 shadow-2xl overflow-hidden">
              {/* Header campaña */}
              <div className="bg-green-700 px-6 py-4 flex items-center justify-between">
                <div>
                  <p className="text-green-200 text-xs font-semibold uppercase tracking-wide">Modo Campaña</p>
                  <p className="text-white font-bold">
                    {campanaIndex + 1} / {prospectosCampana.length}
                  </p>
                </div>
                <button
                  onClick={() => setModoCampana(false)}
              ...
              <div className="h-1.5 bg-slate-700">
                <div
                  ...
                  style={{ width: `${((campanaIndex + 1) / prospectosCampana.length) * 100}%` }}
                />
              </div>
              ...
              <div className="p-6 space-y-4">
                <div>
                  <h2 className="text-xl font-bold text-white">{prospectoActualCampana.nombre}</h2>
                  <div className="flex flex-wrap gap-2 mt-1">
                    <span className={`px-2 py-0.5 rounded text-xs ${getTipoColor(prospectoActualCampana.tipo)}`}>
                      {getTipoTexto(prospectoActualCampana.tipo)}
                    </span>
                    <span className="text-slate-400 text-sm">{prospectoActualCampana.zona} · {prospectoActualCampana.provincia}</span>
                  </div>
                </div>

                <div className="bg-slate-700 rounded-xl p-4">
                  <p className="text-xs text-slate-400 uppercase font-semibold mb-1">Teléfono</p>
                  <p className="text-green-400 text-lg font-bold">{prospectoActualCampana.telefono}</p>
                  {prospectoActualCampana.direccion && (
                    <p className="text-slate-300 text-sm mt-1">{prospectoActualCampana.direccion}</p>
                  )}
                </div>

                {/* Preview del mensaje */}
                <div className="bg-slate-700/50 rounded-xl p-3 border border-slate-600">
                  <p className="text-xs text-slate-400 uppercase font-semibold mb-1">Mensaje a enviar</p>
                  <p className="text-slate-200 text-sm leading-relaxed">
                    {getMensajeWA(prospectoActualCampana.nombre)}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs ${getEstadoColor(prospectoActualCampana.estado)}`}>
                    {getEstadoTexto(prospectoActualCampana.estado)}
                  </span>
                </div>
              </div>
              ...
              <div className="px-6 pb-6 space-y-2">
                <button
                  onClick={async () => {
                    await contactarProspecto(prospectoActualCampana);
                    // Avanzar al siguiente
                    if (campanaIndex + 1 < prospectosCampana.length) {
                      setCampanaIndex(i => i + 1);
                    } else {
                      setModoCampana(false);
                      alert(`¡Campaña completada! Contactaste ${prospectosCampana.length} prospectos.`);
                    }
                  }}
                  className="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl flex items-center justify-center gap-2 text-base"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                  Contactar y pasar al siguiente →
                </button>

                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      // Saltar sin contactar
                      if (campanaIndex + 1 < prospectosCampana.length) {
                        setCampanaIndex(i => i + 1);
                      } else {
                        setModoCampana(false);
                    ...
                    className="flex-1 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-xl text-sm font-medium"
                  >
                    Saltear →
                  </button>
                  {prospectoActualCampana.googleMaps && (
                    <a
                      href={prospectoActualCampana.googleMaps}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-4 py-2.5 bg-blue-700 hover:bg-blue-600 text-white rounded-xl text-sm font-medium flex items-center gap-1"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 flex gap-2">
                    <button
                      onClick={() => {
                        contactarProspecto(prospectoDetalle);
                        setProspectoDetalle({ ...prospectoDetalle, estado: prospectoDetalle.estado === 'nuevo' ? 'contactado' : prospectoDetalle.estado });
                      }}
