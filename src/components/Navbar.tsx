...
export function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 h-[72px] ${
      scrolled || !isHome
        ? "glass shadow-md border-b border-slate-200/50 dark:border-slate-700/50"
        : "bg-transparent"
    }`}>
      <div className="max-w-7xl mx-auto px-4 h-full">
        <div className="flex items-center justify-between h-full">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-left">
                <div className="py-2">
                  <Link
                    href="/catalogo"
                  ...
                  {categorias.map((cat) => (
                    <div key={cat.id} className="relative group/sub">
                      <Link
                        href={`/seccion/${cat.id}`}
                        className="flex items-center justify-between px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-slate-700 dark:text-slate-200"
                      >
                        <span>{cat.nombre}</span>
                        {/* Si hay subcategorías, mostrar flecha */}
                      ...
                      {cat.subcategorias && cat.subcategorias.length > 0 && (
                        <div className="absolute left-full top-0 ml-1 w-56 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all">
                          <div className="py-2">
                            {cat.subcategorias.map((sub) => (
                              <Link
                                key={sub.id}
                                href={`/seccion/${cat.id}?sub=${sub.id}`}
                                className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-slate-600 dark:text-slate-300"
                              >
        ...
        {/* Menú Mobile */}
        {menuOpen && (
          <div className="md:hidden absolute top-[72px] left-0 right-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shadow-lg z-40">
